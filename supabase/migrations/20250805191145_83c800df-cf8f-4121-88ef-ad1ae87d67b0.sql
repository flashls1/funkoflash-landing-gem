...
-- Also ensure we have proper policies for user_activity_logs
CREATE POLICY IF NOT EXISTS "Admins can manage all activity logs"
ON public.user_activity_logs
FOR ALL
USING (has_role(auth.uid(), 'admin'::app_role))
WITH CHECK (has_role(auth.uid(), 'admin'::app_role));

CREATE POLICY IF NOT EXISTS "Staff can manage all activity logs"
ON public.user_activity_logs
FOR ALL
USING (has_role(auth.uid(), 'staff'::app_role))
WITH CHECK (has_role(auth.uid(), 'staff'::app_role));