-- COMPREHENSIVE BUSINESS EVENT VISIBILITY FIX
...
-- Junction: event ↔ business (already exists, but ensure constraints)
CREATE TABLE IF NOT EXISTS business_event_account (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Make FK constraint (only for events that have business_event_id populated)
-- Don't make it NOT NULL yet as some calendar events might not be business events
...
-- Link business users to their business accounts based on email matching
...
PERFORM log_security_event(
  'business_event_visibility_fix_migration',
  'business_events',
  NULL,
  NULL,
  jsonb_build_object(
    'migration_timestamp', now(),
    'description', 'Comprehensive business event visibility fix with proper data model and RLS policies'
  )
);