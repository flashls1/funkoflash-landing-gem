...
-- Remove conflicting/past policies (ignore if absent)
drop policy if exists business_users_can_view_assigned_business_events on public.business_events;
drop policy if exists business_users_can_view_assigned_calendar_events on public.calendar_event;
drop policy if exists "Users with calendar:view can view events" on public.calendar_event;
...
begin
  select id into v_user from public.profiles where lower(email)=lower(p_user_email);
