    ...
    const fetchData = async () => {
      try {
        ...
        const {
          data: talentData,
          error: talentError
        } = await supabase.rpc('get_public_talent_showcase');
        if (talentError) {
          ...
          if (session) {
            const {
              data: fallbackData,
              error: fallbackError
            } = await supabase.from('talent_profiles').select('id, name, slug, headshot_url, bio, sort_rank').eq('active', true).eq('public_visibility', true).order('sort_rank', {
              ascending: true
            });
            if (!fallbackError) {
      ...
      backgroundAttachment: 'fixed'
        ...
        <main className="container mx-auto px-4 py-8">
          ...
          {loading ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {[...Array(8)].map((_, i) => <Card key={i} className="overflow-hidden bg-card/80 backdrop-blur-sm border-border">
              ...
              {talents.map(talent => <Card key={talent.id} className="overflow-hidden hover:shadow-xl transition-shadow bg-card/80 backdrop-blur-sm border-border">
                  <div className="aspect-square relative overflow-hidden">
                    {talent.headshot_url ? <img src={getImageUrl(talent.headshot_url)} alt={`${talent.name} headshot`} className="w-full h-full object-cover" /> : <div className="w-full h-full bg-muted flex items-center justify-center">
                  ...
                  <CardContent className="p-4">
                    <h3 className="text-lg font-semibold text-foreground mb-2 my-0">
                      {talent.name}
                    </h3>
                    <Button variant="default" size="sm" className="w-full" onClick={() => window.location.href = `/talent/${talent.slug}`}>
          ...
          {!loading && talents.length === 0 && <div className="text-center py-12">
